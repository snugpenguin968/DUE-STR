...
class StrSumo_Meta_collector_plus:
    ...
    def run(self):
                ...
                for vehicle_id in arrived_at_destination:
                    if vehicle_id in self.controlled_vehicles:
                        ...
                        if not arrived_at_destination:
                            print("destination:",self.controlled_vehicles[vehicle_id].destination,"did not arrive now at:",self.controlled_vehicles[vehicle_id].current_edge)
                        print("Vehicle {} reaches the destination: {}, timespan: {}, deadline missed: {}".format(vehicle_id, arrived_at_destination, time_span, miss))
                        if not arrived_at_destination:
        ...
        """csv2Data('./History/Test_Results.csv')
        temp = []
        overall= []
