...
from controller.ShortestTime import ShortestTimeController
from controller.FloydWarshall import FWController
from controller.SelflessController import SelflessPolicy_Reader
...
control_v_min = 500 #number of controlled cars
control_v_max = 1000
uncontrol_v = 50
...
def ShortestTime(vehicles,file):
    print('Testing Shortest Time Controller')
    scheduler = ShortestTimeController(init_connection_info, file, Round_name)
    run_simulation_Meta_Plus(scheduler, vehicles)
    del scheduler

def test_FloydWarshall(vehicles,file):
    print('Testing Floyd Warshall Controller')
    scheduler = FWController(init_connection_info, file, Round_name)
    run_simulation_Meta_Plus(scheduler, vehicles)
    del scheduler

def test_selfless_policy(vehicles, file):
    print('Testing selfless algorithm')
    scheduler = SelflessPolicy_Reader(init_connection_info, file, Round_name)
    run_simulation_Meta_Plus(scheduler, vehicles)
    del scheduler
...
if __name__ == "__main__":
    ...
    for increment in range(round_min,round_max): #,control_v,
        ...
        #ShortestTime(tom_vehicles2,file_name)
        #test_dijkstra_policy(vehicles)
        #test_FloydWarshall(vehicles,file_name)
        test_selfless_policy(vehicles,file_name)
