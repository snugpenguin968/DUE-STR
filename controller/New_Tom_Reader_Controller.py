...
class NewPolicy_Reader(RouteController):
    ...
    routes = {} # = to route with regards to directions from current edge is returned by make decisions
       ...
       # print(test)
        ...
        for vehicle in vehicles:
            ...
            if vehicle.vehicle_id not in self.routes.keys():
                #print(vehicle.vehicle_id+" not in routes")
            ...
            if local_targets[vehicle.vehicle_id] not in self.routes_edges[vehicle.vehicle_id]:
                print(vehicle.vehicle_id+" NOT IN ROUTES EDGES")
                ...
                #self.routes.update({vehicle.vehicle_id: self.bellman_ford(vehicle, connection_info)})
